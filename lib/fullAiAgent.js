...
(function (global) {
  ...
  const SLOT_KEYWORDS = {
    name: [
      ...
      'nickname', 'nick name', 'pen name', 'pseudonym', 'alias',
      'first name', 'last name', 'user name', 'username',
      '姓名', '昵称', '名字', '称呼', '笔名',
      '名前', 'なまえ', 'お名前', 'ニックネーム',
      'ihr name',
      'ime', 'jméno', 'meno',
  ...
  function buildStepsFromSnapshot(snapshotResult, fillData) {
    ...
    // 全量填充：尝试对未匹配的 input 进行推测填充
    const urlLikeKeywords = ['url', 'website', 'site', 'web', 'homepage', 'blog', 'link', '网址', '链接'];
    const emailLikeKeywords = ['email', 'e-mail', 'mail', '邮箱', 'メール', '이메일'];
    for (const n of inputs) {
      if (used.has(n.id) || n === humanVerificationNode) continue;
      const combined = norm([n.name, n.placeholder, n.labelText].filter(Boolean).join(' '));
      if (!combined) continue;
      if (n.inputType === 'email' && fillData.commentEmail) {
        used.add(n.id);
        steps.push({ op: 'fill', uid: n.id, value: fillData.commentEmail });
      } else if (n.inputType === 'url' && fillData.commentWebsite) {
        used.add(n.id);
        steps.push({ op: 'fill', uid: n.id, value: fillData.commentWebsite });
      } else if (emailLikeKeywords.some(kw => combined.includes(kw)) && fillData.commentEmail) {
        used.add(n.id);
        steps.push({ op: 'fill', uid: n.id, value: fillData.commentEmail });
      } else if (urlLikeKeywords.some(kw => combined.includes(kw)) && fillData.commentWebsite) {
        used.add(n.id);
        steps.push({ op: 'fill', uid: n.id, value: fillData.commentWebsite });
      } else if (scoreSlot(combined, SLOT_KEYWORDS.name) && fillData.commentName) {
        used.add(n.id);
        steps.push({ op: 'fill', uid: n.id, value: fillData.commentName });
      }
    }
